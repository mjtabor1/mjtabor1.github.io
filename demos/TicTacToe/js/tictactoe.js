// This function will get fired once the DOM is loaded--> window.onload=.
...
function rollForTurn() {
	var xArray = []; //initialize array
	var ranNum = '';
	var minimum = 1;
	var maximum = 11;
    var first = ""; // whoever wins the rolls gets added here
    var txt1 = ""; // text that pops up within the gameMsgBox
	for (var i = 0; i < 2; i++) {
		// random whole number between 1 and 10
		ranNum = Math.floor(Math.random()*(maximum - minimum) + minimum);
		xArray.push(ranNum); // pushes ranNum into the xArray
	}
...
function startGame() {
	...
	showPlayer.innerHTML = activePlayer; //changes the active player content to the assigned player after the rollForTurn() function
...
function determineAvatar() {
	...
	return paintAvatar; // returned back the correct avatar to whoever calls it
}
